...
angular.module("app").controller("transferInwardViewCtrl", ["sharedSvc", "$state", "$rootScope", "toastr", function (sharedSvc, $state, $rootScope, toastr) {
  ...
  function submitTasks() {
    ...
    userJob.PalletDetailModel = userJob[0].tasks;
    userTaskRepository.save({}, userJob[0], function (response) {
      vm.isBusy = false;
      vm.isBusy2 = false;
      vm.formData = {};
  ...
  function endJob(docNo) {
    ...
    userTaskRepository.update({}, {}, function (response) {
      ...
      // $rootScope.userJob.TransferInwardModel.GoodReceiveNotes = $rootScope.userJob.TransferInwardModel.GoodReceiveNotes.filter(x => x.DocumentNo !== docNo);
