(function () {
  ...
  angular.module("app").controller("transferInwardNewCtrl", ["sharedSvc", "$state", '$scope', "$rootScope", "toastr", "barcodeService", function (sharedSvc, $state, $scope, $rootScope, toastr, barcodeService) {
    ...
    function getProductInfo(trfInward) {
      ...
      if (task === null || task === undefined) { // user does not have a task already
        ...
        if (task.jobs === undefined || task.jobs === null) {  // user has task but  there's no job already
          ...
          var currentJob = task.jobs.filter(function (x) {
            return x.type === 'inward' && x.documentNo == vm.transferInwards[0].DocumentNo
          });
          if (currentJob.length === 0) {  // this particular document is not among the documents started
            ...
            for (var i = 0; i < task.jobs.length; i += 1) {
              if (task.jobs[i].type === "inward" && task.jobs[i].documentNo === vm.transferInwards[0].DocumentNo) {
                currentIndex = i;
                break;
              }
            }
