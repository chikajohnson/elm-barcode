(function () {
  "use strict";
  angular.module("app").controller("transferInwardEditCtrl", ["sharedSvc", "$state", '$scope', '$rootScope', "toastr", function (sharedSvc, $state, $scope, $rootScope, toastr) {
    var vm = this;
    // alert("enterd TRF edit");
    vm.job = {};
    vm.formData = {};
    vm.products = [];
    vm.batches = [];
    ...
    // alert("about to load  TRF");
    if (allUserJob && allUserJob.TransferInwardModel) {
      // alert(allUserJob.TransferInwardModel.DocCount);
      vm.transferInwards = allUserJob.TransferInwardModel.GoodTransferInwards.filter(function (item) {
        return item.DocumentNo === vm.currentDoc;
      });

      if (vm.transferInwards.length > 0) {
        // alert("theres is a transfer of docNo " + vm.transferInwards[0].DocumentNo);
      }
      else {
    ...
    if (vm.transferInwards[0]) {
      // alert("transfer inward has value " + vm.transferInwards[0].DocumentNo);
    }
    ...
    getProductInfo(vm.transferInwards[0]);
    loadInward();
    ...
    function getProductInfo(inw) {
      ...
      if (inw === undefined || inw === null || inw === '') {
        ...
        inw.GoodTransferInwardDetails.forEach(function (prod) {
    ...
    vm.setSelectedBatch = function (item) {
      ...
      if (item !== undefined || item !== null) {
        ...
        var batches = vm.batches.filter(function (b) {
          return b.BatchID === item;
        });
    ...
    vm.save = function () {
      ...
      // alert("filter  TRF details");
      //get goood transfer note detail from which the pallet comes from
      var noteDetails = vm.transferInwards[0].GoodTransferInwardDetails.filter(function (x) {
      ...
      //check if the pallette number about to save already exisits in local storage
      // var exists = false;
      // vm.job.jobs.forEach(function (job) {
      //   for (var index = 0; index < vm.job.jobs.length; index++) {
      //     if (vm.job.jobs[index].documentNo === vm.currentDoc) {
      //       var currTasks = job.tasks.filter(function(t){
      //           return t.lotNo === vm.formData.lotNo;
      //       });
      ...
      if (vm.job && vm.job.jobs) {
        ...
        if (results.length > 0) {
          ...
          if (vm.job.currentTask !== undefined || vm.job.currentJob !== null) {
            ...
            // vm.batches = vm.batches.filter(function(b){
            //   return b.ProductID === vm.product;
            // });

            // vm.productMeasures = vm.productMeasures.filter(function(m){
            //   return m.ProductID === vm.product;
            // });
          }
        }
        else {
